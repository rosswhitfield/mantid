...
function(mtd_add_sip_module)
  set(options)
  set(oneValueArgs MODULE_NAME TARGET_NAME SIP_SRC MODULE_OUTPUT_DIR PYQT_VERSION FOLDER)
  set(multiValueArgs
      HEADER_DEPS
      SYSTEM_INCLUDE_DIRS
      INCLUDE_DIRS
  ...
  # Create the module spec file from the list of .sip files The template file expects the variables to have certain
  # names
  ...
  if(PARSED_PYQT_VERSION EQUAL 5)
    if(SIP_BUILD_EXECUTABLE)
      ...
      _add_sip_library_v4(${PARSED_TARGET_NAME} ${PARSED_MODULE_NAME} ${_module_spec} _sip_include_deps)
    endif()
  ...
  # Suppress Warnings about sip bindings have PyObject -> PyFunc casts which is a valid pattern GCC8 onwards detects GCC
  # 8 onwards needs to disable functional casting at the Python interface
  ...
  target_include_directories(${PARSED_TARGET_NAME} SYSTEM PRIVATE ${PARSED_SYSTEM_INCLUDE_DIRS} ${Python_INCLUDE_DIRS})
  ...
  set_target_properties(${PARSED_TARGET_NAME} PROPERTIES LIBRARY_OUTPUT_NAME ${PARSED_MODULE_NAME})
  set_target_properties(${PARSED_TARGET_NAME} PROPERTIES CXX_CLANG_TIDY "")

  if(PARSED_MODULE_OUTPUT_DIR)
    set_target_properties(${PARSED_TARGET_NAME} PROPERTIES LIBRARY_OUTPUT_DIRECTORY ${PARSED_MODULE_OUTPUT_DIR})
  endif()
  ...
  if(WIN32)
    ...
    if(PYTHON_DEBUG_LIBRARY)
      set_target_properties(${PARSED_TARGET_NAME} PROPERTIES LIBRARY_OUTPUT_NAME_DEBUG ${PARSED_TARGET_NAME}_d)
    endif()
...
# ~~~
function(_add_sip_library target_name module_name module_spec pyqt_major_version sip_include_deps_var)
  # first produce template files for the sip-build project in the binary directory
  ...
  # add command for running sip-build. Sets a custom build directory and sip-build is then responsible for the directory
  # layout below that. We are just interested in the directory that contains the generated .cpp file
  set(_sip_build_dir sip-generated)
  set(_sip_generated_cpp ${_project_dir}/${_sip_build_dir}/${module_name}/sip${module_name}part0.cpp)
  ...
  add_custom_command(
    ...
    DEPENDS ${module_spec} ${_project_dir}/project.py ${_project_dir}/pyproject.toml ${_sip_include_deps}
            ${_sip_sanitizer}
...
#                           dependencies to the target
# ~~~
function(_add_sip_library_v4 target_name module_name module_spec sip_include_deps_var)
