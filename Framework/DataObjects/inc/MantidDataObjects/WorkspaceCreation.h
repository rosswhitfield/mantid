...
std::unique_ptr<T> createRagged(const P &parent) {
  ...
  for (size_t i = 0; i < numHistograms; ++i) {
    ...
    ws->setSharedX(i, parent.sharedX(i));
    // ws->setHistogram(i, parent.histogram(i));
    // ws->setHistogram(i, std::move(std::make_unique<HistogramData::Histogram>(parent.sharedX(i))));
    // ws->setHistogram(i, std::make_unique<HistogramData::Histogram>(parent.sharedX(i)));
  }
  return ws;
}
