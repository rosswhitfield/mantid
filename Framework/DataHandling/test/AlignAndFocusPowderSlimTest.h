...
public:
  ...
  void test_split_full_time() {
    ...
    createSplitter->setProperty("DataX", std::vector<double>{0.2, 0.202, 0.204, 0.206, 0.208, 0.21, 0.212});
    createSplitter->setProperty("DataY", std::vector<double>{0, 1, 2, 0, 1, 2});
    ...
    ws = LoadEventNexus("VULCAN_218062.nxs.h5", NumberOfBins=1, FilterByTimeStop=1)
    grp = CreateGroupingWorkspace(ws, GroupDetectorsBy='bank')
    ws = GroupDetectors(ws, CopyGroupingFromWorkspace="grp")

    times = [0.2, 0.202, 0.204, 0.206, 0.208, 0.21, 0.212]
    targets = [0, 1, 2, 0, 1, 2]
