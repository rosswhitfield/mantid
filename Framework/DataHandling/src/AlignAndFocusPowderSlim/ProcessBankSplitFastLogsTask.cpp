...
ProcessBankSplitFastLogsTask::ProcessBankSplitFastLogsTask(
    ...
    const std::map<detid_t, double> &calibration, const std::set<detid_t> &masked, const size_t events_per_chunk,
    const size_t grainsize_event, const std::vector<PulseROI> pulse_indices,
    const std::map<DateAndTime, int> splitterMap, std::shared_ptr<API::Progress> &progress)
