...
//---------------------------------------------------------------------------------------------------------------------------
// LegacyNexusDescriptor public methods
//---------------------------------------------------------------------------------------------------------------------------
/**
...
LegacyNexusDescriptor::LegacyNexusDescriptor(const std::string &filename)
  ...
  if (filename.empty()) {
    throw std::invalid_argument("LegacyNexusDescriptor() - Empty filename '" + filename + "'");
  }
  if (!std::filesystem::exists(filename)) {
    throw std::invalid_argument("LegacyNexusDescriptor() - File '" + filename + "' does not exist");
  }
  try {
    // this is very expesive as it walk the entire file
    initialize(filename);
  } catch (::NeXus::Exception &e) {
    throw std::invalid_argument("LegacyNexusDescriptor::initialize - File '" + filename +
...
//---------------------------------------------------------------------------------------------------------------------------
// LegacyNexusDescriptor private methods
//---------------------------------------------------------------------------------------------------------------------------
 ...
 */
void LegacyNexusDescriptor::walkFile(::NeXus::File &file, const std::string &rootPath, const std::string &className,
