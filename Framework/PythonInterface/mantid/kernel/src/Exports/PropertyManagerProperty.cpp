...
void export_PropertyManagerProperty() {
  ...
  class_<PropertyManagerProperty, bases<BaseClassType>, boost::noncopyable>("PropertyManagerProperty", no_init)
      .def(init<const std::string &, const unsigned int>(
          (arg("self"), arg("name"), arg("direction") = Direction::Input), "Construct an PropertyManagerProperty"))
      .def("__init__", make_constructor(&createPropertyManagerPropertyWithDict, default_call_policies(),
