...
void InstrumentWidgetDecoder::decode(const QMap<QString, QVariant> &map, InstrumentWidget &obj,
                                     const QString &projectPath, const bool loadMask) {
...
void InstrumentWidgetDecoder::decodeMaskTab(const QMap<QString, QVariant> &map, InstrumentWidgetMaskTab *obj) {
  ...
  obj->m_ring_rectangle->setChecked(activeTools["ringRectangleButton"].toBool());
...
void InstrumentWidgetDecoder::decodeRenderTab(const QMap<QString, QVariant> &map, InstrumentWidgetRenderTab *obj) {
  ...
  obj->m_displayDetectorsOnly->setChecked(map[QString("displayDetectorsOnly")].toBool());
  ...
  surface->setShowPeakRelativeIntensityFlag(map[QString("showRelativeIntensity")].toBool());
...
void InstrumentWidgetDecoder::decodeTreeTab(const QMap<QString, QVariant> &map, InstrumentWidgetTreeTab *obj) {
...
void InstrumentWidgetDecoder::decodePickTab(const QMap<QString, QVariant> &map, InstrumentWidgetPickTab *obj) {
...
void InstrumentWidgetDecoder::decodeBinMasks(const QList<QVariant> &list, MaskBinsData &obj) {
...
void InstrumentWidgetDecoder::decodeSurface(const QMap<QString, QVariant> &map,
  ...
  QColor qColor(color[QString("red")].toInt(), color[QString("green")].toInt(), color[QString("blue")].toInt(),
...
void InstrumentWidgetDecoder::decodeProjection3D(const QMap<QString, QVariant> &map, Projection3D &obj) {
  this->decodeViewPort(map[QString("viewport")].toMap(), obj.m_viewport);
}

void InstrumentWidgetDecoder::decodeViewPort(const QMap<QString, QVariant> &map, Viewport &obj) {
...
void InstrumentWidgetDecoder::decodeMaskShapes(const QList<QVariant> &list, Shape2DCollection &obj) {
...
Shape2D *InstrumentWidgetDecoder::decodeShape(const QMap<QString, QVariant> &map) {
  ...
  QColor qColor(color1[QString("red")].toInt(), color1[QString("green")].toInt(), color1[QString("blue")].toInt(),
                color1[QString("alpha")].toInt());

  shape->setColor(qColor);

  QMap<QString, QVariant> color2 = map[QString("fillColor")].toMap();
  QColor qFillColor(color2[QString("red")].toInt(), color2[QString("green")].toInt(), color2[QString("blue")].toInt(),
                    color2[QString("alpha")].toInt());
  shape->setFillColor(qFillColor);

  return shape;
}

Shape2D *InstrumentWidgetDecoder::decodeEllipse(const QMap<QString, QVariant> &map) {
...
Shape2D *InstrumentWidgetDecoder::decodeSector(const QMap<QString, QVariant> &map) {
  ...
  return new Shape2DSector(innerRadius, outerRadius, startAngle, endAngle, QPointF(centerX, centerY));
}

Shape2D *InstrumentWidgetDecoder::decodeFree(const QMap<QString, QVariant> &map) {
  QPolygonF polygon;
...
void InstrumentWidgetDecoder::decodeAlignmentInfo(const QList<QVariant> &list,
  ...
  for (const auto &item : list) {
    ...
    Mantid::Kernel::V3D qValue(qLabMap[QString("x")].toDouble(), qLabMap[QString("y")].toDouble(),
